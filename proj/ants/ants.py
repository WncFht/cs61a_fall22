...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            if (place.ant.is_container and self.is_container) or \
            (not place.ant.is_container and not self.is_container) or \
            (not place.ant.can_contain(self) and not self.can_contain(place.ant)):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        attack_place = None
        pos = 0
        while not temp.is_hive:
            if len(temp.bees) > 0 and self.lower_bound <= pos <= self.upper_bound:
                attack_place = temp
                break
            temp = temp.entrance
            pos += 1
        if not attack_place:
            return None
        return random_bee(attack_place.bees)  # REPLACE THIS LINE
        # END Problem 3 and 4
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while temp != None:
            if temp.ant != None:
                temp.ant.double()
                if temp.ant.is_container and temp.ant.ant_contained != None:
                    temp.ant.ant_contained.double()
            temp = temp.exit
    ...
    def insects_in_front(self):
        ...
        while not temp.is_hive:
            for bee in temp.bees:
                res[bee] = distance
            if temp.ant is not None:
                if temp == self.place:
                    if isinstance(temp.ant, LaserAnt) or \
                    (temp.ant.is_container and isinstance(temp.ant.ant_contianed, LaserAnt)):
                        distance += 1
                        temp = temp.entrance
                        continue
                    res[temp.ant] = distance
                else:
                    if temp.ant.is_container and temp.ant.ant_contianed is not None:
                        res[temp.ant.ant_contained] = distance
                    res[temp.ant] = distance
            distance += 1
            temp =temp.entrance
        return res
